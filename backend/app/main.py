...
async def startup():
    ...
    # Verify voice telephony readiness for Agent #1028
    try:
        from app.services.snapserve_service import SnapServeService
        agent = await SnapServeService.get_agent(SnapServeService.PERMANENT_AGENT_ID)
        if agent:
            logger.info(f"🎙️ Stellar Agri Voice Advisor (Agent #{SnapServeService.PERMANENT_AGENT_ID}) is active and ready for inbound/outbound calls.")
    except Exception as e:
        logger.warning(f"Voice agent startup health check notice: {e}")
